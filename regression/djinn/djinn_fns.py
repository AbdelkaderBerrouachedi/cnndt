...
def get_hyperparams(regression, ttn, xscale, yscale, x1, y1, dropout_keep_prob, 
    ...
    for lriter in range(0, 2): #iterate twice
        ...
        for pp in range(0, len(lrs)):
            ...
            if (random_state): tf.compat.v1.set_random_seed(random_state)

            x = tf.compat.v1.placeholder("float32",[None, n_input], name="input")
            ...
            for i in range(1, nhl+1):
                biases['h%s'%i] = tf.Variable(tf.random.normal([int(n_hidden[i])], mean=0.0, 
